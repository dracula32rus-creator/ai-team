  ...
  return (
    <>
      ...
      <div style={{
        ...
        <div style={{
          padding: "16px 20px", borderBottom: "1px solid #ffffff0a",
          ...
          {!selectedMember ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {memberStats.map(({ member, done, active, overdue, total, rate }) => (
                <div key={member.username} onClick={() => setSelectedMember(member)} style={{
                  background: member.bg, border: `1px solid ${member.color}33`,
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                    {[
                      ...
                      { label: "Просроч.", value: overdue, color: overdue > 0 ? "#FF4444" : "#444" },
                    ].map(s => (
                ...
                <>
                  ...
                  <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
                    {[{ label: "Активные", value: false }, { label: "Архив", value: true }].map(tab => (
                      <button key={String(tab.value)} onClick={() => setShowArchived(tab.value)} style={{
...
export default function TasksPage() {
  ...
  const pad = isMobile ? "0 12px" : "0 40px";
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#060606", color: "#fff", fontFamily: "'SF Pro Display', -apple-system, sans-serif" }}>
      ...
      <button onClick={() => setPanelOpen(true)} style={{
        ...
        background: "#1a1a1a", border: "1px solid #333", borderRight: "none",
        borderRadius: "8px 0 0 8px", color: "#888", padding: isMobile ? "12px 6px" : "16px 8px",
        cursor: "pointer", zIndex: 95, display: "flex", flexDirection: "column",
      ...
      <div style={{
        ...
        display: "flex", alignItems: "center", justifyContent: "space-between",
        position: "sticky", top: 0, zIndex: 50, background: "#06060699", backdropFilter: "blur(20px)",
      }}>
        <div>
        ...
        <div style={{ display: "flex", gap: isMobile ? 12 : 20, alignItems: "center" }}>
          {[
            ...
            { label: "Актив.", value: activeTasks, color: "#5B8CFF" },
            { label: "Готово", value: doneTasks, color: "#00C896" },
            { label: "Просроч.", value: overdueTasks, color: overdueTasks > 0 ? "#FF4444" : "#444" },
          ].map(stat => (
          ...
          {!isMobile && (
            <div style={{ background: "#ffffff08", border: "1px solid #ffffff11", borderRadius: 8, padding: "6px 12px", fontSize: 12, color: "#888" }}>
      ...
      <div style={{ padding: isMobile ? "12px 12px 0" : "20px 40px 0" }}>
        {!isMobile && <div style={{ fontSize: 12, color: "#555", marginBottom: 10 }}>
          {selectedMember ? `Задачи: ${selectedMember}` : "Все задачи"}
        </div>}
        <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 6 }}>
          <div onClick={() => setSelectedMember(null)} style={{
            ...
            fontSize: 12, color: selectedMember === null ? "#fff" : "#666",
          }}>Все</div>
          {TEAM.map(member => {
            ...
            return (
              <div key={member.username} onClick={() => setSelectedMember(isSelected ? null : member.name)} style={{
                ...
                <div style={{ position: "relative", width: 28, height: 28, flexShrink: 0 }}>
                  ...
                  <img src={member.avatar} alt={member.name} style={{
                    position: "absolute", inset: isSelected ? 3 : 0,
                    borderRadius: "50%", width: isSelected ? "calc(100% - 6px)" : "100%",
                    height: isSelected ? "calc(100% - 6px)" : "100%",
                    objectFit: "cover", border: `2px solid ${isSelected ? member.color : "#ffffff22"}`,
                  }} />
                </div>
                <div>
      ...
      <div style={{ padding: isMobile ? "10px 12px" : "12px 40px", display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <button onClick={handleCloseWeek} style={{
          background: "#00C89611", border: "1px solid #00C89633", borderRadius: 8,
          padding: "7px 12px", color: "#00C896", fontSize: 12, cursor: "pointer",
          display: "flex", alignItems: "center", gap: 4,
        }}>
          {closing ? "⏳" : "✅"} {isMobile ? "Закрыть неделю" : (closing ? "Закрываю..." : "Закрыть неделю")}
        </button>
      </div>

      {/* Мобильный переключатель колонок */}
      {isMobile && (
        <div style={{ display: "flex", gap: 6, padding: "0 12px 10px", overflowX: "auto" }}>
          {STATUS_COLUMNS.map(col => (
            <button key={col.key} onClick={() => setActiveCol(col.key)} style={{
              flexShrink: 0, padding: "6px 12px", borderRadius: 8, cursor: "pointer",
              background: activeCol === col.key ? col.color + "22" : "#ffffff08",
              border: activeCol === col.key ? `1px solid ${col.color}66` : "1px solid #ffffff0a",
              color: activeCol === col.key ? col.color : "#888",
              fontSize: 12, fontWeight: 600,
            }}>
            ...
            return (
              <div key={col.key} style={{ background: "#111", borderRadius: 12, overflow: "hidden" }}>
                ...
                <div style={{ padding: 10, display: "flex", flexDirection: "column", gap: 8, minHeight: isMobile ? "auto" : 200 }}>
                  {colTasks.length === 0 && <div style={{ color: "#333", fontSize: 12, textAlign: "center", marginTop: 24 }}>Нет задач</div>}
                  {colTasks.map(task => {
                      ...
                      <div key={task.id} style={{
                        ...
                        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                          {STATUS_COLUMNS.filter(s => s.key !== col.key && s.key !== "просрочена").map(s => (
                            <button key={s.key} onClick={() => updateStatus(task.id, s.key)} style={{
